...
export default function Home() {
  ...
  // Fetch messages once both user and activeSessionId are ready
  useEffect(() => {
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    if (!input.trim() || isLoading || isImageLoading || !user?.sub || !activeSessionId) return;
    ...
    try {
      ...
      const aiMsg = { role: "assistant", content: res.text || "(No response)" };
      ...
      setMessages((msgs) => [
        ...msgs,
        { role: "assistant", content: "Sorry, there was an error with the AI response." },
      ]);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleImageGen = async () => {
    ...
    try {
      ...
      setMessages((msgs) => [...msgs, aiImageMsg]);
      ...
      setMessages((msgs) => [
        ...msgs,
        { role: "assistant", content: "Sorry, there was an error with image generation." },
      ]);
    } finally {
      ...
      <div className="flex-grow-1 d-flex flex-column">
        ...
        >
          <div className="d-flex align-items-center me-auto">
            ...
            <h5 className="mb-0 fw-semibold" style={{ color: '#f0f6fc' }}>AI Chat Assistant</h5>
          </div>
          ...
          <div className="mx-auto" style={{ maxWidth: '800px' }}>
            {messages.length === 0 && (
              <div className="text-center py-5">
                ...
                <h3 className="mb-3" style={{ color: '#f0f6fc' }}>How can I help you today?</h3>
                <p className="text-muted mb-0">Start a conversation with your AI assistant</p>
              </div>
            )}
            ...
            {messages.map((msg, i) => (
              ...
              >
                ...
                <div
                  className={`px-4 py-3 rounded-4 position-relative ${
                    msg.role === "user" 
                      ? "text-white" 
                      : "text-white"
                  }`}
                ...
                {msg.role === "user" && (
                  ...
                  >
                    <i className="bi bi-person-fill text-white" style={{ fontSize: '14px' }} />
                  </div>
                )}
              </div>
            ))}
        ...
        >
          ...
          `}</style>
          <form onSubmit={handleSend} className="mx-auto" style={{ maxWidth: '800px' }}>
              ...
              >
                ...
                <div className="d-flex gap-2 flex-shrink-0">
                  <button
                    ...
                    style={{
                      backgroundColor: input.trim() && !isLoading && !isImageLoading ? '#6f42c1' : '#30363d',
                      border: 'none',
                      borderRadius: '8px',
                      minWidth: '90px',
                      height: '36px',
                      color: input.trim() && !isLoading && !isImageLoading ? '#fff' : '#8b949e',
                      fontSize: '13px',
                      fontWeight: '500'
                    }}
                  >
                  ...
                  <button
                    ...
                    style={{
                      backgroundColor: input.trim() && !isLoading && !isImageLoading ? '#238636' : '#30363d',
                      border: 'none',
                      borderRadius: '8px',
                      minWidth: '80px',
                      height: '36px',
            ...
            <div className="d-flex justify-content-between align-items-center mt-2 px-1">
              <small style={{ color: '#8b949e', fontSize: '13px' }}>
                Press <kbd className="px-2 py-1 rounded" style={{ backgroundColor: '#21262d', color: '#f0f6fc', fontSize: '12px' }}>Shift</kbd> + <kbd className="px-2 py-1 rounded" style={{ backgroundColor: '#21262d', color: '#f0f6fc', fontSize: '12px' }}>Enter</kbd> for a new line
              </small>
              <small style={{ color: '#8b949e', fontSize: '13px' }}>
